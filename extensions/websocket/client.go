...
type setMeta func(*conn)

func dial(u *neturl.URL, options *clientOptions) (rawConnection, error) {
	switch u.Scheme {
	case "ws":
		_, addr := createHostPortAddress(u.Host, ":80")
		c, err := tnet.DialTCP("tcp", addr, options.timeout)
		if err != nil {
			return nil, err
		}
		c.SetFlushWrite(true) // enable flushwrite for websocket.
		return c, nil
	case "wss":
		hostname, addr := createHostPortAddress(u.Host, ":443")
